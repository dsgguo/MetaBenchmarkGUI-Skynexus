...
with safe_import_context() as import_ctx:
    ...
    from metabci.brainda.datasets import Weibo2014
...
class Dataset(BaseDataset):
    ...
    parameters = {
        'duration': [i for i in np.arange(3, 7, 0.5)],
        'subject': [i for i in range(1, 11)]  # 1 to 110
    }
    ...
    def get_data(self):
        ...
        paradigm = create_mi_paradigm(
            channels=channels,
            events=['left_hand', 'right_hand', "hands", "feet", "left_hand_right_foot", "right_hand_left_foot"],
            interval=[(3, 7)],
            srate=srate
        )
        ...
        if dataset.dataset_code == "weibo2014":
