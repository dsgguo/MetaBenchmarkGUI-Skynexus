...
def generate_filterbank(
    ...
    subband components.
    创建滤波器组，即获得可以将输入信号划分为多个子带分量的带通滤波器系数。
    update log:
        2023-12-10 by Leyi Jia <18020095036@163.com>, Add code annotation
    ...
    order : int
        Filter order.
        滤波器的阶数（Order），决定了滤波器的陡峭程度和复杂度。阶数越高，滤波器的过渡带（通带与阻带之间的过渡区域）越窄，但同时计算成本也越高。
    rp : float
        The maximum ripple allowed in the passband below the unit gain is 0.5 by default.
        指的是通带的最大波动（Ripple in Passband），通常以分贝（dB）计。
        这意味着通带内信号的最大增益或衰减不会超过1dB，保证了信号在这个频段内的保真度
    ...
    """
    filterbank = []#每个滤波器都是一个二阶环节序列（Second-Order Section, SOS）形式的带通滤波器,SOS格式适合于稳定且具有高精度的滤波器实现。
    for wp, ws in zip(passbands, stopbands):
        if order is None:
            N, wn = cheb1ord(wp, ws, 3, 40, fs=srate)
            sos = cheby1(N, rp, wn, btype="bandpass", output="sos", fs=srate)#最佳阶数 `N`
        else:
